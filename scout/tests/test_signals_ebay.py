"""
test_signals_ebay.py — scout/signals/ebay.py (Session 55, key-gated optional eBay active-listing
comps — renamed from "sold-comps" in the 2026-07-06 review: the Browse API this module calls
returns currently ACTIVE listings, not completed sales; see module docstring).
...
class ActiveListingCompsSkipTest(unittest.TestCase):
    def test_skips_honestly_without_app_id(self):
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("EBAY_APP_ID", None)
            result = ebay.active_listing_comps("012345678905")
...
class ActiveListingCompsLiveShapeTest(unittest.TestCase):
    def test_computes_active_count_and_median_price(self):
        ...
        with mock.patch.dict(os.environ, {"EBAY_APP_ID": "x", "EBAY_CERT_ID": "y"}), \
            ...
            result = ebay.active_listing_comps("012345678905", token="fake-token")
...
class EbayFeaturesTest(unittest.TestCase):
    def test_ratio_computed_when_ok(self):
        with mock.patch.object(ebay, "active_listing_comps", return_value={
            "status": "ok", "active_count": 5, "median_price": 15.0, "listings": []}):
            feats = ebay.ebay_features("012345678905", amazon_price=20.0)
        self.assertEqual(feats["ebay_active_listing_count"], 5)
        self.assertEqual(feats["median_active_price_vs_amazon_ratio"], 0.75)
        self.assertFalse(feats["ebay_stale"])

    def test_nullable_and_stale_when_skipped(self):
        with mock.patch.object(ebay, "active_listing_comps", return_value={
            "status": "skipped", "active_count": None, "median_price": None, "listings": []}):
            feats = ebay.ebay_features("012345678905", amazon_price=20.0)
        self.assertIsNone(feats["ebay_active_listing_count"])
        self.assertIsNone(feats["median_active_price_vs_amazon_ratio"])
